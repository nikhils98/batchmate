...
describe('cost matrix route planner', () => {
  ...
  describe('plan is invoked with dummy locations', () => {
    ...
    beforeAll(() => {
      matrixComputeSpy = jest.spyOn(matrixComputeContext, 'compute').mockReturnValue([
        [1, 2, 3, 4],
        [1, 2, 3, 4],
      ]);
    ...
    const origins: Location[] = [{ id: 'depot', point: { lat: 24.55, lng: 67.23 } }];
    const destinations: Location[] = [
      { id: '1', point: { lat: 24.6, lng: 67 } },
      { id: '2', point: { lat: 25, lng: 67.5 } },
      { id: '3', point: { lat: 23.8, lng: 68 } },
    ];
